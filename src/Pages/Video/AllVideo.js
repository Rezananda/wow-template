...
const AllVideo = () => {
  ...
  const [sort, setSort] = useState("sort[0]=video_name:asc");
  ...
  const [selectedSort, setSelectedSort] = useState("sort[0]=video_name:asc");
  ...
  const handleTemplateMobile = async () => {
    ...
    try {
      await axios
        .get(
          `${process.env.REACT_APP_API_URL}/api/tutorial-and-tips?${
            filter.length > 0
              ? `${filter
                  .map(
                    (el, index) =>
      ...
      setErrorVideo(getErrorMessage(error));
    } finally {
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      setErrorFilter(getErrorMessage(error));
    } finally {
    ...
    <>
        ...
        <Layout className={"mb-6 hidden md:block"}>
          <div className="flex w-full justify-between items-center border-b border-gray-200 py-3">
            ...
            <div className="flex items-center font-custom gap-2">
              ...
              <span className="flex items-center px-4 py-2 rounded-lg bg-gray-100 border border-gray-300">
                ...
                >
                  <option value="sort[0]=video_name:asc">A - Z</option>
                  <option value="sort[0]=video_name:desc">Z - A</option>
          ...
          <div className="flex flex-row space-x-4 mt-2 relative">
            <div className="basis-1/6">
              ...
              {errorFilter !== null ? (
                <ErrorHandler error={errorFilter} onRetry={handleFilter} /> //error handle
              ) : (
                <>
                  {loadingFilter ? (
                    <p>Loading...</p>
                        ...
                        {video.data.length > 0 ? (
                          video?.data.map((val, index) => (
                            <CardVideo
                              ...
                              category={
                                val.attributes.tutorial_and_tip_category.data
                                  .attributes.desc
                              }
                              id={val.id}
                              count_view={val.attributes.count_view}
                              handleRefreshVideo={handleVideo}
                            />
                          ))
                        ) : (
                          <Label
                      ...
                      {video.data.length > 0 && (
                        <div className="flex w-full justify-end gap-2">
                          ...
                          <Button
                            ...
                            size={"medium"}
                            className={
                              video.meta.pagination.page ===
                                video.meta.pagination.pageCount &&
                              "!text-blue-300 hover:!no-underline"
                            }
                            onClick={() =>
                              video.meta.pagination.page ===
        ...
        <Layout className={"mb-6 md:hidden block"}>
          <div className="flex justify-between w-full border-b border-gray-200 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {selectedSort !== "sort[0]=video_name:asc" && (
          ...
          <div className="flex flex-row space-x-4 mt-2 relative">
            <div className="">
              ...
              {errorVideo !== null ? (
                <ErrorHandler error={errorVideo} onRetry={handleVideo} /> //error handle
              ) : (
                <>
                  {loadingTemplate ? (
      ...
      >
        <Button
          ...
          onClick={() => setSelectedSort("sort[0]=video_name:asc")}
        >
          Hapus
        </Button>
        <div className="flex flex-col gap-2 border rounded-lg px-4 py-2">
          <div className="flex items-center">
            <input
              ...
              checked={selectedSort === "sort[0]=video_name:asc"}
              onChange={(e) => setSelectedSort(e.target.id)}
            />
            <label
              htmlFor="sort[0]=video_name:asc"
            ...
            >
              <span
                className={`border border-gray-300 rounded-full mr-2 w-5 h-5 ${
                  selectedSort === "sort[0]=video_name:asc" ? "bg-blue-500" : ""
                }`}
              ></span>
              A - Z
            </label>
          ...
          <div className="flex items-center">
            <input
              id="sort[0]=video_name:desc"
              type="radio"
              name="radio-vertical-group"
              className="hidden checked:bg-no-repeat checked:bg-center checked:border-blue-500 checked:bg-blue-500"
              checked={selectedSort === "sort[0]=video_name:desc"}
              onChange={(e) => setSelectedSort(e.target.id)}
            />
            <label
              htmlFor="sort[0]=video_name:desc"
              className="flex items-center cursor-pointer text-gray-600 text-xl font-normal"
            >
              <span
                className={`border border-gray-300 rounded-full mr-2 w-5 h-5 ${
                  selectedSort === "sort[0]=video_name:desc"
                    ? "bg-blue-500"
                    : ""
                }`}
              ></span>
